...
def test_change_root_repoints_same_identity_and_preserves_state(tmp_path):
    ...
    try:
        ...
        assert os.path.normcase(old_root) in before
        ...
        assert os.path.normcase(new_root) in after                   # root now points at the new location
        assert os.path.normcase(old_root) not in after
...
def test_change_root_refuses_package_id_mismatch(tmp_path):
    ...
    try:
        ...
        assert os.path.normcase(old_root) in detail
        assert os.path.normcase(other_root) not in detail
    finally:
        _stop(wb)
